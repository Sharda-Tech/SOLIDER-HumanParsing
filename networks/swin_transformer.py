...
class SwinTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        #self.context_encoding = PSPModule(768, 512) # tiny & small
        #self.context_encoding = PSPModule(1024, 512) # base
        if context_in_channel == 768:
            self.edge = Edge_Module(in_fea=[96, 192, 384], mid_fea=192, out_fea=2) # tiny & small
        else:
       ...
       #     nn.Conv2d(1024, 256, kernel_size=1, padding=0, dilation=1, bias=False), # base
            nn.BatchNorm2d(256),
    ...
    def forward(self, x):
        ...
        x, hw_shape = self.patch_embed(x) # b, h*w, c
        if self.use_abs_pos_embed:
        ...
        for i, stage in enumerate(self.stages):
            ...
            #print('=',i,x.shape,hw_shape,out.shape,out_hw_shape)
            if self.semantic_weight >= 0:
...
def _swin_base_patch4_window7_224(img_size=224,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
...
def swin_base_patch4_window7_224(img_size=224,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    ...
    model = SwinTransformer(pretrain_img_size = img_size, patch_size=4, window_size=7, embed_dims=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), context_in_channel=1024, fusion_in_channel=1024, drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, **kwargs)
    #print(kwargs)
...
def swin_small_patch4_window7_224(img_size=224,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    ...
    model = SwinTransformer(pretrain_img_size = img_size, patch_size=4, window_size=7, embed_dims=96, depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), context_in_channel=768, fusion_in_channel=832, drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, **kwargs)
    #print(kwargs)
    settings = pretrained_settings['swin']['imagenet']
    initialize_pretrained_model(model, settings, **kwargs_bak)
    return model
...
def swin_tiny_patch4_window7_224(img_size=224,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    ...
    model = SwinTransformer(pretrain_img_size = img_size, patch_size=4, window_size=7, embed_dims=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), context_in_channel=768, fusion_in_channel=832, drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, **kwargs)
    #print(kwargs)
    settings = pretrained_settings['swin']['imagenet']
    initialize_pretrained_model(model, settings, **kwargs_bak)
    return model

def initialize_pretrained_model(model, settings, pretrained='', **kwargs):
